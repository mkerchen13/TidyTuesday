...
Plot2 <- Game_Data_Top5 %>%
  ...
  facet_wrap(~gamename)+  # Facet wraps the plot by game name 
  theme(legend.position = "bottom",  # Places the legend on the bottonm to save space on the image
        legend.title = element_text(size = 12,     # Changes the size a color of the plot title
                                    color = "black"),
